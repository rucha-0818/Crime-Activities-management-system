...
def forget_pass():
    def change_password():
        if user_entry.get()=='' or newpass_entry.get()=='' or confirmpass_entry.get()=='':
            ...
            con = pymysql.connect(host='localhost', user='root', password='root',database='userdata1')
            mycursor = con.cursor()
    ...
    heading_lebal=Label(window, text='RESET PASSWORD',font=('arial',18,'bold'),
                        bg='white', fg='magenta2')
    heading_lebal.place(x=480,y=60)
...
def login_user():
    if usernameEntry.get()=='' or passwordEntry.get()=='':
        ...
        try:
            ...
            messagebox.showerror('Error', 'Connection is not establishedtry again')
            return
...
heading=Label(login_window,text='USER LOGIN',font=('Microsft Yahei UI Light',23,'bold')
              ,bg='white',fg='firebrick1')
heading.place(x=605,y=120)

usernameEntry=Entry(login_window,width=25,font=('Microsft Yahei UI Light',11,'bold')
                    ,bd=0,fg='firebrick1')
...
passwordEntry=Entry(login_window,width=25,font=('Microsft Yahei UI Light',11,'bold')
                    ,bd=0,fg='firebrick1')
...
forgetButton=Button(login_window,text='Forgot Password?',bd=0,bg='white',activebackground='white'
                 ,cursor='hand2',font=('Microsft Yahei UI Light',9,'bold'),
